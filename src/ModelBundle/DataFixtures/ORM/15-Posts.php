...
class Posts extends AbstractFixture implements OrderedFixtureInterface
{
    ...
    public function load(ObjectManager $manager)
    {
        ...
        $p1->setContent('Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis sagittis, ligula nec facilisis
...
habitasse platea dictumst. Cras ut consequat ipsum. Donec faucibus ac arcu ut rutrum.');
        $p1->setUser($this->getUser($manager, 'David'));
        $p2 = new Post();
        $p2->setTitle('Donec in turpis ac dolor tempor egestas nec ac tortor');
        $p2->setContent('Donec in turpis ac dolor tempor egestas nec ac tortor. Suspendisse facilisis ipsum id neque
...
aliquet leo. Donec vitae nulla lobortis est efficitur faucibus eget at urna.');
        $p2->setUser($this->getUser($manager, 'Alex'));
        $p3 = new Post();
        $p3->setTitle('Suspendisse facilisis dapibus placerat');
        $p3->setContent('Suspendisse facilisis dapibus placerat. Sed sed tellus magna. Maecenas eget vehicula lorem, sit
        ...
        $p3->setUser($this->getUser($manager, 'Alex'));
        $manager->persist($p1);
        $manager->persist($p2);
        $manager->persist($p3);
        $manager->flush();
